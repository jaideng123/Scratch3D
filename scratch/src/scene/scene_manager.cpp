...
unsigned int scratch::SceneManager::handleSelection(scratch::Shader &selectionShader, glm::vec2 mousePosition) {
    ...
    for (auto currentNode : activeScene.rootNode.getChildren()) {
        ...
        if (currentEntity->hasComponent<scratch::ModelRenderable>()) {
            ...
            std::vector<std::shared_ptr<scratch::Mesh>> meshesToRender = renderableComponent->getMeshes();
...
//        std::shared_ptr<scratch::EntityComponent> entityComponent;
//        if ((*itr)["type"].GetString() == scratch::ModelRenderable::TYPE_NAME) {
...
//            }
//            entityComponent = std::make_shared<scratch::ModelRenderable>((*itr)["id"].GetUint(), linkedModel, materials);
//        } else {
